...
const TabsList = <T extends ValidComponent = "div">(
  ...
  return (
    <TabsPrimitive.List
      class={cn(
        "relative inline-flex h-10 items-center justify-center rounded-full bg-card/40 border border-border/60 p-1 text-muted-foreground backdrop-blur-sm",
        local.class
      )}
      {...others}
    />
...
const TabsIndicator = <T extends ValidComponent = "div">(
  ...
  return (
    <TabsPrimitive.Indicator
      class={cn(
        "duration-250ms absolute transition-all data-[orientation=horizontal]:bottom-[4px] data-[orientation=vertical]:right-[4px] data-[orientation=horizontal]:h-[3px] data-[orientation=vertical]:w-[3px] rounded-full bg-primary/50",
        local.class
      )}
      {...others}
    />
