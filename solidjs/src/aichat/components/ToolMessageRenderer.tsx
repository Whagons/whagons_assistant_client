  ...
  return (
    <>
      ...
      <Show when={message.role === "tool_result"}>
        <Show when={index() > 0 && messages()[index() - 1]?.role === "tool_call"}>
          {(() => {
            ...
            return (
              <div class={`transition-opacity duration-500 ease-in-out ${isMounted() ? 'opacity-100' : 'opacity-0'} md:max-w-[900px] w-full px-4 my-2`}>
                <div class="w-full rounded-md border">
                  ...
                  >
                    <div class="flex items-center space-x-2">
                      <div class="w-6 h-6 rounded-full bg-primary-foreground  flex items-center justify-center text-white shadow-sm ring-2 dark:ring-white/20  ring-black/20">
                        ...
                        stroke={`${theme() === 'dark' ? 'white' : 'black'}`} stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round">
                          <path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/>
                        </svg>
                      </div>
                      <span class="text-primary font-medium">{formattedToolName}</span>
                    </div>
                    <div class="flex items-center">
                      ...
                      <svg
                        ...
                        class={`size-4 shrink-0 transition-transform duration-200 ml-2 ${isOpen() ? 'rotate-180' : ''}`}
                      >
                        <path d="M6 9l6 6l6 -6" />
                      </svg>
                    </div>
                  ...
                  >
                    <div class="p-3 space-y-2">
                      <div>
                        <h4 class="text-xs font-medium text-primary/80 mb-1">Call Details:</h4>
                        <JsonSyntaxHighlighter content={toolCallContent} />
                      </div>
                      <div>
                        <h4 class="text-xs font-medium text-primary/80 mb-1">Result:</h4>
