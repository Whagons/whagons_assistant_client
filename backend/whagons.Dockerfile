# Whagons Assistant Backend
# Uses: Desarso/whagons_assistant_config
#
# Build args required:
#   GH_TOKEN - GitHub token with repo read access
#
# In Coolify: set Dockerfile path to "backend/whagons.Dockerfile"
...
RUN if [ -z "$GH_TOKEN" ]; then echo "ERROR: GH_TOKEN build arg is required" && exit 1; fi && \
    git clone https://x-access-token:${GH_TOKEN}@github.com/Desarso/whagons_assistant_config.git /tmp/config
