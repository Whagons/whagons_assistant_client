...
@dataclass
class ReasoningPart:
    """A reasoning response from a model."""
    ...
    def has_content(self) -> bool:
        """Return `True` if the reasoning content is non-empty."""
...
ModelResponsePart = Annotated[Union[TextPart, ToolCallPart, ReasoningPart], pydantic.Discriminator('part_kind')]
...
class ReasoningPartDelta:
    """A partial update (delta) for a `ReasoningPart` to append new reasoning content."""

    reasoning_delta: str
    """The incremental reasoning content to add to the existing `ReasoningPart` content."""

    part_delta_kind: Literal['reasoning'] = 'reasoning'
    """Part delta type identifier, used as a discriminator."""

    def apply(self, part: ModelResponsePart) -> ReasoningPart:
        """Apply this reasoning delta to an existing `ReasoningPart`.

        Args:
            part: The existing model response part, which must be a `ReasoningPart`.

        Returns:
            A new `ReasoningPart` with updated reasoning content.

        Raises:
            ValueError: If `part` is not a `ReasoningPart`.
        """
        if not isinstance(part, ReasoningPart):
            raise ValueError('Cannot apply ReasoningPartDeltas to non-ReasoningParts')
        return replace(part, reasoning=part.reasoning + self.reasoning_delta)
...
ModelResponsePartDelta = Annotated[Union[TextPartDelta, ToolCallPartDelta, ReasoningPartDelta], pydantic.Discriminator('part_delta_kind')]
