...
ManagedPart = Union[ModelResponsePart, ToolCallPartDelta, ReasoningPartDelta]
"""
...
class ModelResponsePartsManager:
    ...
    def handle_reasoning_delta(
        ...
        *,
        ...
        """Handle incoming reasoning content, creating or updating a ReasoningPart in the manager as appropriate.

        Args:
            vendor_part_id: The ID the vendor uses to identify this piece
                of reasoning. If None, a new part will be created unless the latest part is already
                a ReasoningPart.
            reasoning: The reasoning content to append to the appropriate ReasoningPart.

        Returns:
            A `PartStartEvent` if a new part was created, or a `PartDeltaEvent` if an existing part was updated.

        Raises:
            UnexpectedModelBehavior: If attempting to apply reasoning content to a part that is
                not a ReasoningPart.
        """
        existing_reasoning_part_and_index: tuple[ReasoningPart, int] | None = None

        if vendor_part_id is None:
            # If the vendor_part_id is None, check if the latest part is a ReasoningPart to update
            if self._parts:
                part_index = len(self._parts) - 1
                latest_part = self._parts[part_index]
                if isinstance(latest_part, ReasoningPart):
                    existing_reasoning_part_and_index = latest_part, part_index
        else:
            # Otherwise, attempt to look up an existing ReasoningPart by vendor_part_id
            part_index = self._vendor_id_to_part_index.get(vendor_part_id)
            if part_index is not None:
                existing_part = self._parts[part_index]
                if not isinstance(existing_part, ReasoningPart):
                    raise UnexpectedModelBehavior(
                        f"Cannot apply a reasoning delta to {existing_part=}"
                    )
                existing_reasoning_part_and_index = existing_part, part_index

        if existing_reasoning_part_and_index is None:
            # There is no existing reasoning part that should be updated, so create a new one
            new_part_index = len(self._parts)
            part = ReasoningPart(reasoning=reasoning)
            if vendor_part_id is not None:
                self._vendor_id_to_part_index[vendor_part_id] = new_part_index
            self._parts.append(part)
            return PartStartEvent(index=new_part_index, part=part)
        else:
            # Update the existing ReasoningPart with the new reasoning delta
            existing_reasoning_part, part_index = existing_reasoning_part_and_index
            part_delta = ReasoningPartDelta(reasoning_delta=reasoning)
