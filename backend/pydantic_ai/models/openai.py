...
class OpenAIModel(Model):
    ...
    async def request(
        ...
        response = await self._completions_create(
            messages,
            False,
            cast(OpenAIModelSettings, model_settings or {}),
    ...
    def _get_tools(
        ...
        if model_request_parameters.result_tools:
            tools += [
                self._map_tool_definition(r)
                for r in model_request_parameters.result_tools
            ]
        return tools
        ...
        if isinstance(part.content, str):
            ...
            for item in part.content:
                if isinstance(item, str):
                    ...
                    audio = InputAudio(
                        data=base64_encoded, format=response.headers.get("content-type")
                    )
                    content.append(
                        ChatCompletionContentPartInputAudioParam(
                            input_audio=audio, type="input_audio"
...
class OpenAIResponsesModel(Model):
    ...
    def _process_response(self, response: responses.Response) -> ModelResponse:
        ...
        for item in response.output:
            if item.type == "function_call":
                items.append(
                    ToolCallPart(item.name, item.arguments, tool_call_id=item.call_id)
                )
        return ModelResponse(items, model_name=response.model, timestamp=timestamp)
    ...
    async def _process_streamed_response(
        ...
        return OpenAIResponsesStreamedResponse(
            ...
            _timestamp=datetime.fromtimestamp(
                first_chunk.response.created_at, tz=timezone.utc
            ),
        )
    ...
    def _get_reasoning(
        ...
        if reasoning_effort is None and reasoning_generate_summary is None:
            return NOT_GIVEN
        return Reasoning(
            effort=reasoning_effort, generate_summary=reasoning_generate_summary
        )
    ...
    ) -> tuple[str, list[responses.ResponseInputItemParam]]:
...
class OpenAIStreamedResponse(StreamedResponse):
    ...
    async def _get_event_iterator(self) -> AsyncIterator[ModelResponseStreamEvent]:
        async for chunk in self._response:
            ...
            if (hasattr(choice.delta, "reasoning") and choice.delta.reasoning is not None
                and choice.delta.reasoning != ""
            ):
                yield self._parts_manager.handle_reasoning_delta(
                    vendor_part_id="reasoning", reasoning=choice.delta.reasoning
                )
