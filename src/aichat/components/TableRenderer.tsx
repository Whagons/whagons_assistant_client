...
const TableRenderer: React.FC<TableRendererProps> = (props) => {
  ...
  return (
    <div className="w-full overflow-x-auto">
      <table className="w-full table-fixed table-rounded !border-0 !border-separate !border-spacing-0">
        <colgroup>
        ...
        <thead>
          <tr className="bg-gray-800/80 dark:bg-gray-800 text-white">
            {tableData.headers.map((header, index) => (
              <th key={index} className="px-6 py-4 text-left text-sm font-semibold tracking-wide truncate">
                {header}
          ...
          {tableData.rows.map((row, rowIndex) => (
            <tr key={rowIndex} className="hover:bg-gray-50/50 dark:hover:bg-gray-700/30 transition-colors">
              {row.cells.map((cell, cellIndex) => (
                <td key={cellIndex} className="px-6 py-4 text-sm text-gray-900 dark:text-gray-100 align-top whitespace-normal break-words">
                  {cell}
                </td>
              ))}
            </tr>
