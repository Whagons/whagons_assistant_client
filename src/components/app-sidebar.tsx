...
export function AppSidebar() {
  ...
  const renderChatSection = (title: string, chatsToRender: Conversation[]) => {
    ...
    return (
      <div className="mb-4">
        <div className="text-xs font-bold uppercase tracking-[0.18em] text-muted-foreground px-3 mb-2">
          {title}
        </div>
        <div className="space-y-1">
          {chatsToRender.map((chat) => (
            <div key={chat.id} className="group/menu-item relative">
              <Link
                to={`/chat/${chat.id}`}
                className={`group/link relative flex items-center gap-2 pl-3 pr-10 py-2 text-base md:text-[13px] font-medium rounded-md hover:bg-sidebar-accent w-full overflow-hidden transition-colors ${
                  currentChatId === chat.id
                    ? "bg-sidebar-accent text-sidebar-foreground shadow-[inset_3px_0_0_var(--primary)]"
              ...
              <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center justify-end text-gray-500 dark:text-gray-400 transition-transform transform translate-x-[102%] group-hover/menu-item:translate-x-0 group-hover/menu-item:pointer-events-auto">
                <div className="relative z-10 flex items-center pr-1 bg-transparent rounded-r-md gap-1">
                  {pinnedChatIds.includes(chat.id) ? (
                    <button
                      className="rounded-md p-1.5 hover:bg-sidebar-accent hover:text-primary"
                      title="Unpin chat"
                    ...
                    <button
                      className="rounded-md p-1.5 hover:bg-sidebar-accent hover:text-primary disabled:opacity-50"
                      title="Pin chat"
  ...
  return (
    <>
      <Sidebar collapsible="offcanvas" side="left" variant="sidebar">
        <SidebarContent className="bg-sidebar flex flex-col h-screen">
          <div className="p-3 flex flex-col items-center">
            <img src={logoUrl} alt="Whagons" className="h-[42px] w-auto object-contain" />
            <Link
              to="/chat/"
      ...
      {chatToDelete && (
        ...
        >
          <div 
            className="bg-card text-card-foreground border border-border rounded-lg shadow-xl p-6 m-4 max-w-sm w-full"
          ...
          >
            ...
            <div className="flex justify-end space-x-3">
              <button
                onClick={() => setConfirmDeleteChatId(null)}
                className="px-4 py-2 rounded-md text-sm font-medium text-secondary-foreground bg-secondary hover:bg-secondary/80"
              >
                Cancel
              </button>
              <button
